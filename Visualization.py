...
class loadFile(QDialog):
    global InputEdges, InputPoints
    InputEdges, InputPoints = '',''
    ...
    def OpenPointFile(self):
        ...
        InputPoints = QFileDialog().getOpenFileName(None,"Open file", "", "Text Files (*.txt)")[0]
        self.label1.setText('Selected file: ' + str(InputPoints))

    def OpenEdgeFile(self):

        #output： C:/Users/ZZ/PycharmProject/.../InputPoints.txt
        InputEdges = QFileDialog().getOpenFileName(None,"Open file", "", "Text Files (*.txt)")[0]
        self.label2.setText('selected file: ' + str(InputEdges))
    ...
    def validate(self):
        ...
        if InputPoints!='':
            pointsIsValid = False
            with open(InputPoints, 'r') as f_in1:
                for line in f_in1:
                    records1 = line.split()
                    if len(records1) != 3:
        ...
        if InputEdges!='':
            edgeIsValid = False
            with open(InputEdges, 'r') as f_in2:
                for line in f_in2:
                    records2 = line.split()
